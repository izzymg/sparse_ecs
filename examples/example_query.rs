...
fn spawn_world() -> World {
    ...
    for i in 0..10 {
        let e = world.spawn();
        world.get_mut::<Position>().unwrap().add_entity(
            Position {
                x: i as f32,
                y: 0.0,
            },
        ...
        if i % 2 == 0 {
            world
                .get_mut::<Velocity>()
                .unwrap()
                .add_entity(Velocity { dx: 1.0, dy: 0.5 }, e);
        }
        if i % 3 == 0 {
            world
                .get_mut::<Health>()
                .unwrap()
                .add_entity(Health(100), e);
...
fn main() {
    ...
    if let Some(hp_set) = world.get::<Health>() {
        if let Some(h) = hp_set.get(first_entity) {
            println!("Entity0 Health: {}", h.0);
        }
    }
    if let Some(mana_set) = world.get::<Mana>() {
        if let Some(m) = mana_set.get(first_entity) {
            println!("Entity0 Mana: {}", m.0);
        }
    }
}
